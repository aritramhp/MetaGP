...
parser.add_argument('--app',dest='apptainer_file', type=str, default='bash',
                    help='Apptainer file (.sif).')
...
docker_cmd = 'bash ' if args.apptainer_file=='bash' else \
            'docker run --rm -v '+input_basedir+':/mnt/data metagpcrcdocker ' if args.apptainer_file=='docker' else \
...
# make config file
config_file = metagp_core.make_config_file(output_dir)
